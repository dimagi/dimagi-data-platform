...
b as (select visit_id, date_part('epoch',time_start - lag(time_end, 1) 
OVER (PARTITION BY visit.user_id ORDER BY time_start)) AS time_since_previous from visit order by visit.user_id, time_start)

SELECT visit.visit_id, users.user_id, cases.case_id, time_start, time_end, form_duration, a.total_forms,
coalesce(b.time_since_previous, -1) as time_since_previous, users.domain
...
WHERE visit.user_id = users.id
and form_visit.form_id = form.id
