...
from (select id, user_id from users where username is not null or id in (select max(id) from users group by user_id)
 or id = (select max(id) from users where user_id is null)) as a, users as b
where (a.user_id = b.user_id or (a.user_id is null and b.user_id is null)) and (a.id <> b.id));

update form set user_id = new_id from tmp_user_mapping where form.user_id = old_id;
update cases set user_id = new_id from tmp_user_mapping where cases.user_id = old_id;
update cases set owner_id = new_id from tmp_user_mapping where cases.owner_id = old_id;
update device_log set user_id = new_id from tmp_user_mapping where device_log.user_id = old_id;

--create missing tables
delete from mobile_user_domain;
delete from mobile_user;
insert into mobile_user (select id, groups from users where user_id not in (select user_id from web_user_old) and id not in (select old_id from tmp_user_mapping));
...
insert into web_user (select id from users where user_id in (select user_id from web_user_old) and id not in (select old_id from tmp_user_mapping));
...
delete from users where id in (select old_id from tmp_user_mapping)
